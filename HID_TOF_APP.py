...
class HIDTesterApp:
    def __init__(self, master):
        ...
        self.tof0_var = tk.StringVar(value="N/A")
        self.tof1_var = tk.StringVar(value="N/A")
        self.tof2_var = tk.StringVar(value="N/A")
        self.tof3_var = tk.StringVar(value="N/A")
        self.tof4_var = tk.StringVar(value="N/A")
    ...
    def send_sequence3(self, commands):
        """Generic function to send a list of commands with 1s delay to tab 2."""
        def execute_step(index):
            if index < len(commands):
                self.out_entry2.delete(0, tk.END)
                self.out_entry2.insert(0, commands[index])
                self.send_output_report2()
                self.master.after(200, lambda: execute_step(index + 1))
        execute_step(0)
